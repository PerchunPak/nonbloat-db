...
try:
    ...
    pytestmark = pytest.mark.skip(
        "You didn't install `github_hooks` group, this test depends on this group."
    )
...
def test_calculate_file_content(
    tmp_path: pathlib.Path, faker: faker_package.Faker
) -> None:
...
def test_file_changes_merge(faker: faker_package.Faker) -> None:
    ...
    assert commit_and_push.FileChanges(
        additions=list_1, deletions=list_2
    ).merge(
        commit_and_push.FileChanges(additions=list_3, deletions=list_4)
    ) == commit_and_push.FileChanges(
        additions=list_1 + list_3, deletions=list_2 + list_4
    )
...
def test_str_file_changes_returns_truncated_contains_fields(
    ...
    assert json.loads(
        str(
            commit_and_push.FileChanges(
                additions=[commit_and_push.FileAddition(path, faker.word())]
            )
        )
    )["additions"] == [{"contents": "...", "path": path}]
...
def test_bool_file_changes_return_correct_value(
    ...
    assert bool(
        commit_and_push.FileChanges(
            additions=faker.pylist() if additions else [],
            deletions=faker.pylist() if deletions else [],
        )
...
def test_to_file_changes(
    cls: commit_and_push.NewFile,
    faker: faker_package.Faker,
    mocker: pytest_mock.MockerFixture,
) -> None:
    """Tests that ``to_file_changes`` function returns :class:`.FileChanges` object."""
    mocker.patch(
        "commit_and_push._calculate_file_content", return_value=faker.text()
    )
...
def test_generate_request_data_return_valid_data(
    faker: faker_package.Faker, mocker: pytest_mock.MockerFixture
) -> None:
    """Tests that ``generate_request_data`` function returns valid data."""
    commit = mocker.patch(
        "commit_and_push.get_latest_commit", return_value=faker.sha256()
    )()
    ...
    assert commit_and_push.generate_request_data(
        args, commit_and_push.FileChanges(additions=[], deletions=[])
    ) == (
...
def test_send_http_request_run_code_only_once(
    mocker: pytest_mock.MockerFixture,
) -> None:
...
def test_send_http_request_error_handling(
    mocker: pytest_mock.MockerFixture, faker: faker_package.Faker
) -> None:
    ...
    response.return_value.json.return_value = {
        "errors": [
            {"message": faker.sentence()} for _ in range(faker.pyint(2, 5))
        ]
    }
...
def test_get_welcome_info(
    ...
    assert (
        ...
        == (
            f"Repository to commit and push: '{args.repository}'.\n"  # pyright: ignore[reportAny]
            f"Branch to commit and push: '{args.branch}'.\n"  # pyright: ignore[reportAny]
            f"Commit message: '{args.message}'.\n"  # pyright: ignore[reportAny]
...
def diff_item(
    faker: faker_package.Faker,
) -> typing.Callable[[str | None], git.Diff]:
    ...
    class DiffItem:
        def __init__(self, cls: str | None) -> None:
            ...
            self.change_type = (
                faker.random_element(change_types.values())
                if cls is None
                else change_types[cls]
            )
...
def test_calculate_file_changes_return_file_changes(
    diff_item: typing.Callable[[str | None], git.Diff],
    mocker: pytest_mock.MockerFixture,
) -> None:
    ...
    assert isinstance(
        commit_and_push.calculate_file_changes([diff_item(None)]),
        commit_and_push.FileChanges,
    )
...
def test_calculate_file_changes_modes(
    ...
    if diff.change_type == "A":
        ...
        expected = commit_and_push.DeletedFile(
            path=diff.b_path
        ).to_file_changes()
    elif diff.change_type == "R":
        expected = commit_and_push.RenamedFile(
            new_path=diff.b_path, old_path=diff.a_path
        ).to_file_changes()
    elif diff.change_type == "M":
        expected = commit_and_push.ModifiedFile(
            path=diff.b_path
        ).to_file_changes()
    else:
...
def test_main_calls_right_order(
    mocker: pytest_mock.MockerFixture, faker: faker_package.Faker
) -> None:
    ...
    calculate_file_changes = mocker.patch(
        "commit_and_push.calculate_file_changes"
    )
    generate_request_data = mocker.patch(
        "commit_and_push.generate_request_data", return_value=faker.pytuple()
    )
    send_http_request, commit_url = (
        mocker.patch("commit_and_push.send_http_request"),
        faker.url(),
    )
    send_http_request.return_value = (
        {"data": {"createCommitOnBranch": {"commit": {"url": commit_url}}}},
    )
    ...
    next(main_func)
    generate_request_data.assert_called_once_with(
        parse_args.return_value, calculate_file_changes.return_value
    )
    mocked.pop(0)
    send_http_request.assert_called_once_with(
        *generate_request_data.return_value
    )
    mocked.pop(0)
    assert_not_called(*mocked)
...
def test_main_yields_right_order(
    mocker: pytest_mock.MockerFixture, faker: faker_package.Faker
) -> None:
    ...
    assert next(main_func) == "File changes: " + str(
        calculate_file_changes.return_value
    )
    assert (
        next(main_func)
    ...
    assert (
        next(main_func)
        == f"Congratulations! I'm done! Here is the link to your commit: {commit_url}"
    )
...
def test_main_returns_on_no_changes(
    mocker: pytest_mock.MockerFixture, faker: faker_package.Faker
) -> None:
