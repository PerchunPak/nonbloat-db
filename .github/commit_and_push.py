...
def _create_repo() -> git.Repo:
    ...
    repo.git.add(".")  # pyright: ignore[reportAny]
    return repo


def _calculate_file_content(path: str | Path) -> str:
...
class FileChanges:
    ...
    additions: list[FileAddition] = dataclasses.field(default_factory=list)
    """List of additions in future commit."""
    deletions: list[FileDeletion] = dataclasses.field(default_factory=list)
    ...
    def __str__(self) -> str:
        ...
        for item in result["additions"]:  # pyright: ignore[reportAny]
            item["contents"] = "..."
...
def get_diff() -> git.diff.DiffIndex[git.diff.Diff]:
    """Getter for the diff.

    Returns:
        Diff object.
    """
    return repo.index.diff(None, staged=True)  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
...
def calculate_file_changes(
    diff: git.diff.DiffIndex[git.diff.Diff],
) -> FileChanges | None:
...
def generate_request_data(
...
) -> tuple[tuple[t.Any, ...], dict[t.Any, t.Any]]:
    ...
    return ("https://api.github.com/graphql",), {
        "headers": {
            "Authorization": f"bearer {args.token}",  # pyright: ignore[reportAny]
        ...
        "data": json.dumps(
            {
                ...
                "variables": {
                    "input": {
                        ...
                        "message": {"headline": args.message},  # pyright: ignore[reportAny]
    ...
    Example:
        .. code-block:: python
            for response in send_http_request(("https://api.github.com/graphql",), {}):
                ...
                print(response)
    """
    response = requests.post(*args, **kwargs)  # pyright: ignore[reportAny]  # noqa: S113
    yield response.json()
    response.raise_for_status()
    if "errors" in response.json():
        raise ValueError(
            "Github raised error(s): \n"
            + "\n".join(error["message"] for error in response.json()["errors"])  # pyright: ignore[reportAny]
        )
...
def parse_args() -> (
    ...
    _ = parser.add_argument(
        "repository", help="The repository to commit and push."
    )
    _ = parser.add_argument("branch", help="Branch to commit and push.")
    _ = parser.add_argument("message", help="Commit message.")
    _ = parser.add_argument(
        "--token", "-t", help="GitHub token.", required=True
    )
...
def main() -> c.Iterable[str]:
    ...
    yield (
        f"Congratulations! I'm done! Here is the link to your commit: {response['data']['createCommitOnBranch']['commit']['url']}"  # pyright: ignore[reportPossiblyUnboundVariable]
    )
